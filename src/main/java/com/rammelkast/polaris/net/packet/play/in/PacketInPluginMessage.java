...
@Getter
public final class PacketInPluginMessage extends Packet {
	...
	public void read(final PacketWrapper wrapper) {
		...
		if (this.channel.equals("MC|Brand")) {
			this.data = new byte[wrapper.readVarInt()];
			{
	...
	public void onReceive(final NetClient client) {
		if (this.channel.equals("MC|Brand")) {
			client.getPlayer().setClientBrand(new String(this.data, StandardCharsets.UTF_8));
		}
	}
